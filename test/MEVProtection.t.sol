...
///      mention "manipulation" and 21 mention "attack." Almost every hook proves its code runs;
///      almost none prove the mechanism survives an adversary. That is the gap this file targets.
///
...
contract MEVProtectionTest is HookTest {
    ...
    function setUp() public {
        ...
        deployCodeTo(
            "src/KnotHook.sol:KnotHook", abi.encode(address(manager), address(federation), "Deep", "D", 1), address(deepPool)
        );
        deployCodeTo(
    ...
    /// @dev A sandwich around a victim trade must not extract more than the bound permits. This is
    ///      not Knot's stated target, but a protection that inverts under sandwiching would be a
    ///      real finding, so it is checked rather than assumed.
    function test_evasion_sandwichDoesNotBreakTheBound() public {
        _fund(attacker);
        ...
        _swap(shallowKey, true, 1 ether); // victim

        _swapAs(attacker, shallowKey, false, 2 ether); // back-run
        uint256 a1 = currency0.balanceOf(attacker);

        emit log_named_int("attacker currency0 delta across the sandwich", int256(a1) - int256(a0));
        // The fee is 30 bps a leg, so a round trip is expected to LOSE money absent a real edge.
        assertLt(a1, a0, "the attacker profited on a round trip through a single pool");
    }
    ...
    function test_coalition_attackerOwnedMemberPoolMovesTheAggregate() public {
        ...
        if (quoteAfter > quoteBefore) {
            ...
            emit log_named_uint("loosened by (bps of the original quote)", ((quoteAfter - quoteBefore) * 10_000) / quoteBefore);
        } else {
