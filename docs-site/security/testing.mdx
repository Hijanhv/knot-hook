---
title: "Test suites"
description: "134 tests across 14 suites, and what each one is there to prove."
---

Every suite below runs on `forge test` against the canonical v4 `PoolManager`. Nothing is mocked
except the ERC-20s. Line coverage on `src/` is **96.21%**.
...
| `MEVAdversarial` | 10 | Federation-specific attacks: donation, cross-member sandwich, back-running, multi-block JIT, cycles, ordering independence |
