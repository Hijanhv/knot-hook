---
title: "Test suites"
description: "142 tests across 15 suites, and what each one is there to prove."
---

Every suite below runs on `forge test` against the canonical v4 `PoolManager`. Nothing is mocked
except the ERC-20s. Coverage on `src/` is **96.21% lines, 92.25% statements, 96.43% functions
and 64.81% branches**. The branch figure is the honest one to watch: the uncovered branches are
