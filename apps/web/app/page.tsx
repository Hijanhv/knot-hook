...
import { AGGREGATE_SEED, DEEP_SEED, SHALLOW_SEED, fmt, previewQuote } from "@/lib/knot";

const scan = (address: string) => `${CHAIN.explorer}/address/${address}`;
const fixtureAmount = 5n * 10n ** 18n;
const shallowFixture = previewQuote(fixtureAmount, SHALLOW_SEED, AGGREGATE_SEED, true, true);
const deepFixture = previewQuote(fixtureAmount, DEEP_SEED, AGGREGATE_SEED, true, true);
const quoteCards = [
  {
    t: "Shallow pool",
    s: "bound binds",
    bind: true,
    rows: [
      ["local quote", fmt(shallowFixture.local, 6)],
      ["aggregate quote", fmt(shallowFixture.aggregate, 6)],
      ["enforced", fmt(shallowFixture.enforced, 6)],
      ["withheld → LPs", fmt(shallowFixture.local - shallowFixture.enforced, 6)],
    ],
  },
  {
    t: "Deep pool",
    ...
    bind: false,
    rows: [
      ["local quote", fmt(deepFixture.local, 6)],
      ["aggregate quote", fmt(deepFixture.aggregate, 6)],
      ["enforced", fmt(deepFixture.enforced, 6)],
      ["withheld → LPs", fmt(deepFixture.local - deepFixture.enforced, 6)],
    ],
  },
] as const;
    ...
    <>
          ...
          <div className="relative z-10 lg:pr-10">
            ...
            <div className="mt-9 max-w-lg border-l border-ink/15 pl-5">
              <p className="text-[16.5px] leading-[1.62] text-ink-soft">
                KNOT gives every participating pool a second price boundary: its own curve and a
                virtual curve built from the federation&rsquo;s aggregate reserves. A trade may never
                take the better of both quotes.
              </p>
            </div>
      ...
      <section className="inkfield relative overflow-hidden">
        ...
        <div className="wrap relative py-20 md:py-24">
          ...
          <div className="mt-12 overflow-hidden rounded-lg bg-canvas shadow-lift">
            ...
            Same trade size and manifest seed, reproduced by the contract tests. In this skewed fixture the aggregate
            quote binds; in the proportional control the local quote remains untouched.
              </p>
            </div>
          </div>
      ...
      <section className="relative overflow-hidden border-b border-grid">
        ...
        <div className="wrap relative grid divide-y divide-grid py-14 sm:grid-cols-2 sm:divide-x sm:divide-y-0 lg:grid-cols-4">
          {[
            { v: 6674, u: "bps", l: "boundary in 4× fixture", hl: true },
            { v: 4722, u: "bps", l: "coalition weakening", hl: true },
            { v: RELEASE_EVIDENCE.verification.foundryCases, u: "", l: "passing Foundry cases" },
            { v: RELEASE_EVIDENCE.coverage.lines, u: "%", l: "source line coverage", d: 2 },
          ].map((s, i) => (
      ...
      <section id="problem" className="section scroll-mt-20 border-b border-grid">
        <div className="wrap">
          ...
          <div className="mt-12 grid divide-y divide-grid border-y border-grid md:grid-cols-3 md:divide-x md:divide-y-0">
            {[
              ["Pools price in isolation", "The same token pair can sit behind different depths and reserve ratios."],
              ["One quote can be generous", "A local curve may offer the taker more than the combined reserve curve would."],
              ["KNOT sets a ceiling", "The hook compares both references and enforces the less favourable quote."],
            ].map(([h, p], i) => (
          ...
          <div>
            ...
            <div className="border-t border-grid">
              {CONTRACTS.map(({ name, address }) => (
                <a key={address} href={scan(address)} target="_blank" rel="noopener noreferrer"
              ...
              {!DEPLOYMENT_IS_ACTIVE && (
                <div className="border-b border-grid py-5">
                  <p className="text-[15px] font-medium text-ink">Release manifest unavailable</p>
                  <p className="mt-1 text-sm leading-relaxed text-muted">
                    The verified KNOT addresses are read only from the canonical {CHAIN.name} release manifest.
                  </p>
                </div>
              )}
            </div>
