...
contract DeployDemo is Script {
    ...
    function run() external {
        ...
        // ── federation ──
        KnotFederation federation = new KnotFederation(address(manager), c0, c1, 997, 1000, 8, me);
        ...
        console.log("");
        console.log("Queued. Wait %s block(s), then run script/deploy/DeployDemo.s.sol:Activate", maturity);
        console.log("");
        console.log("--- phase-two inputs ---");
        console.log("export KNOT_FEDERATION=%s", address(federation));
        console.log("export DEEP_POOL=%s", address(deep));
        console.log("export SHALLOW_POOL=%s", address(shallow));
        console.log("export CURRENCY0=%s", c0);
        console.log("export CURRENCY1=%s", c1);
    }
...
contract Activate is Script {
    ...
    function run() external {
        ...
        KnotFederation federation = deep.federation();
        if (
            address(deep) == address(shallow) || address(shallow.federation()) != address(federation)
                || address(deep.poolManager()) != address(shallow.poolManager()) || !federation.isMember(address(deep))
                || !federation.isMember(address(shallow))
        ...
        if (
            d0 != 1000 ether || d1 != 1000 ether || s0 != 100 ether || s1 != 400 ether
                || federation.aggregateReserve0() != d0 + s0 || federation.aggregateReserve1() != d1 + s1
        ) revert SeedMismatch();
        console.log("deep reserves    %s / %s", d0, d1);
        console.log("shallow reserves %s / %s", s0, s1);
        console.log("Activated. Verify the full lifecycle before publishing any address.");
    }
}
