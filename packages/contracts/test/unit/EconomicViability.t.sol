...
contract EconomicViabilityTest is Test {
    ...
    /// @dev Run the KNOT path with the same reserve updates used by `executeSwap`: gross input
    ///      enters both the member and aggregate books and output leaves both before leg two.
    function _knotRoundTripPnl(uint256 stake) internal pure returns (int256 pnl) {
        uint256 aggregate0 = A0;
        uint256 aggregate1 = A1;

        uint256 legA = _knot(stake, S0, S1, aggregate0, aggregate1);
        aggregate0 += stake;
        aggregate1 -= legA;

        uint256 legB = _knot(legA, D1, D0, aggregate1, aggregate0);
        return int256(legB) - int256(stake);
    }
    ...
    function test_thesis_crossPoolRoundTripIsLessProfitableUnderKnot() public {
        ...
        int256 pnlKnot = _knotRoundTripPnl(stake);
    ...
    function test_roundTripDifferencePersistsAcrossSampledSizes() public {
    ...
    ///
    ///      This is a controlled twin-pool counterfactual, not a production routing forecast.
    ///      Retained output and hypothetical forgone fees are reported in one output-token
    ///      numeraire so the comparison is dimensionally valid.
    function test_router_twinPoolCounterfactualUsesOneNumeraire() public {
        uint256 lost;
        uint256 total;
        ...
        emit log_named_uint("output retained in the fixture (token1 wei)", retainedTotal);
        emit log_named_uint("input notional if every quote reroutes (token0 wei)", forgoneVolume);

        uint256 hypotheticalInputFee = (forgoneVolume * 3) / 1000;
        uint256 hypotheticalFeeInToken1 = FullMath.mulDiv(hypotheticalInputFee, A1, A0);
        emit log_named_uint("hypothetical 30bps fee (token1 wei at opening aggregate spot)", hypotheticalFeeInToken1);
        emit log_named_uint(
            "fixture retained/converted-fee ratio (bps)",
            FullMath.mulDiv(retainedTotal, 10_000, hypotheticalFeeInToken1)
        );

        // No assertion treats this fixture as realised economics. It only verifies that the
        // sampled counterfactual and unit conversion are non-empty and reproducible.
        assertGt(total, 0);
        assertGt(hypotheticalFeeInToken1, 0);
    }
