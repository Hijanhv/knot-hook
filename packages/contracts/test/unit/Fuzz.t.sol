...
contract FuzzTest is BaseFixture {
    ...
    function testFuzz_splittingNeverBeatsOneTrade(uint96 raw, uint8 rawSlices) public {
        ...
        try this.externalSwap(true, true, -int256(total)) {}
        catch {
        ...
        for (uint256 i = 0; i < slices; i++) {
            uint256 slice = i + 1 == slices ? total - sent : total / slices;
            sent += slice;
            this.externalSwap(true, true, -int256(slice));
        }
        uint256 sliced = currency1.balanceOf(address(this)) - before;

        assertEq(sent, total, "sliced path did not trade the same total input");
    ...
    function testFuzz_depositThenFullWithdrawalRoundTrips(uint96 rawA, uint96 rawB) public {
        ...
        vm.startPrank(alice);
        try deep.addLiquidity(addParams(a0, a1)) {}
        catch {
            vm.stopPrank();
            return;
        }
        ...
        (,, uint256 activatesAtBlock) = deep.pendingLiquidity(alice);
        ...
        vm.roll(deep.liquidityUnlockBlock(alice));
        vm.prank(alice);
