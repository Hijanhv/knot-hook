...
abstract contract BaseFixture is HookTest {
    ...
    function setUp() public virtual {
        ...
        federation = new KnotFederation(
            address(manager), Currency.unwrap(currency0), Currency.unwrap(currency1), FEE_NUM, FEE_DEN, 4, address(this)
        );
        ...
        (deepKey,) =
            initPool(currency0, currency1, IHooks(address(deep)), LPFeeLibrary.DYNAMIC_FEE_FLAG, SQRT_PRICE_1_1);
        (shallowKey,) =
        ...
        vm.roll(deep.liquidityUnlockBlock(address(this)));
    }
    ...
    function _deployHook(uint160 slot, string memory name, string memory sym) internal returns (KnotHook h) {
        ...
        deployCodeTo(
            ...
            abi.encode(address(manager), address(federation), name, sym, 1),
            address(h)
        );
    }
