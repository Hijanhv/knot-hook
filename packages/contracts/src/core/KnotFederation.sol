...
import {KnotMath} from "../libraries/KnotMath.sol";
...
contract KnotFederation is Ownable2Step {
    ...
    event FederatedSwap(
        ...
        uint256 enforcedAmount
    );

    modifier onlyMember() {
        _requireLiveMember(msg.sender);
        _;
    }
    ...
    constructor(
        ...
        if (manager.code.length == 0) revert InvalidPoolManager();
        ...
        if ((token0 != address(0) && token0.code.length == 0) || token1.code.length == 0) revert InvalidCurrency();
        poolManager = manager;
        currency0 = token0;
        currency1 = token1;
    ...
    // Validation is intentionally explicit here: every branch protects a distinct registration
    // invariant, so splitting it only to reduce a static complexity score would obscure the gate.
    // slither-disable-next-line cyclomatic-complexity
    function register(address hook) external onlyOwner nonReentrant {
        ...
        if (uint160(hook) & Hooks.ALL_HOOK_MASK != REQUIRED_HOOK_FLAGS) revert InvalidMember();
        ...
        try IKnotMember(hook).poolManager() returns (address memberManager) {
            if (memberManager != poolManager) revert InvalidMember();
        } catch {
            revert InvalidMember();
        }
        bytes32 codehash = hook.codehash;
        if (memberCodehash == bytes32(0)) memberCodehash = codehash;
        else if (codehash != memberCodehash) revert InvalidMember();
        isMember[hook] = true;
        memberCount++;
    ...
    /// @notice KNOT always needs an accountable curator for membership and empty-pool recovery.
    /// @dev Ownership may move through Ownable2Step, but cannot be burned accidentally or used
    /// to create a permanently ungoverned federation whose emptied members can never restart.
    ...
    {
        _requireLiveMember(hook);
        return _preview(hook, zeroForOne, exactInput, specifiedAmount);
    }

    function _preview(address hook, bool zeroForOne, bool exactInput, uint256 specifiedAmount)
        private
        view
        returns (uint256 localQuote, uint256 aggregateQuote, uint256 knotQuote)
    {
        ...
        returns (uint256 unspecifiedAmount)
    {
        (uint256 localQuote, uint256 aggregateQuote, uint256 knotQuote) =
            _preview(msg.sender, zeroForOne, exactInput, specifiedAmount);
    ...
    function _requireLiveMember(address hook) private view {
        if (!isMember[hook]) revert NotMember();
        if (hook.codehash != memberCodehash) revert InvalidMember();
    }
}
